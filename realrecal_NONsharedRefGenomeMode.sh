...
"mailx -s '[Support #200] variant identification pipeline' "$redmine,$email""	    exit 1;
        ...
        set +x; echo -e "\n\n" >&2;
        echo "#################################################################################" >&2
        echo "################   STEP0: COPY REF GENOME TO COMPUTE NODE     ###################" >&2
        echo "#################################################################################" >&2
        echo -e "\n\n" >&2; set -x;

        cp $refdir/$ref /dev/shm
	exitcode=$?
	echo `date`
	if [ $exitcode -ne 0 ]
	then
	    MSG="copy file from   $refdir/$ref   to   compute node/dev/null   failed. Exiting now"
            echo -e "program=$scriptfile stopped at line=$LINENO.\nReason=$MSG\n$LOGS" >> $RealignOutputLogs/FAILED_realrecal.${infile}.Anisimov.msg
	    exit $exitcode;
	fi
        ref_local=/dev/shm/$ref
