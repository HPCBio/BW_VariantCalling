...
then
    MSG="$$rootdir/SAMPLENAMES_multiplexed.list file not found. This file is required. exiting now"
    echo -e "program=$scriptfile stopped at line=$LINENO.\nReason=$MSG\n$LOGS" | ssh iforge "mailx -s '[Support #200] variant identification pipeline' "$redmine,$email""
    exit 1;
fi
...
then
    ...
    exit 1;
        ...
        set +x; echo -e "\n\n" >&2; 
        echo -e "####################################### CASE2:   MARKDUPLICATESTOOL == PICARD  OF WITH-QC  SAMPLE              #################" >&2
        echo -e "####################################### part 1: align, then convert sam to bam, then sort, then QC test        #################" >&2
        echo -e "####################################### part 2: mark duplicates, then sort                                     #################" >&2
        echo -e "################################################################################################################################" >&2
        echo -e "####################################### This script performs part 2                                            #################" >&2
        echo -e "\n\n" >&2; set -x;
        cd $outputdir
        echo `date`
        ...
        echo -e "\n\n" >&2; set -x;
        echo -e "#################      WRAP UP: sort by coordinate and index on the fly: required for creating an indexed bam, ###############" >&2
        echo -e "#################      which in turn is required for extracting alignments by chromosome                       ###############" >&2
        echo -e "\n\n" >&2; set -x;
        ...
        set +x; echo -e "\n\n" >&2;        
        echo -e "#######################################################################################################" >&2
        echo -e "########    Done with part 2 of CASE 2                                                          #######" >&2
        echo -e "#######################################################################################################" >&2
        echo -e "\n\n" >&2; set -x;    
