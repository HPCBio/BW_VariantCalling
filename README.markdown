...
The pipeline implements the stages of Figure <img width="100"><https://github.com/HPCBio/BW_VariantCalling/raw/ParameterSweep/media/image05.png> and [?], while allowing different software tools at some of the stages depending on user's preference. These are as shown in table [?] below, and it is assumed that the users would specify the path to each of them in theirhis runfile as shown in section ??.
