...
# The file to read sampleinfo from. This file MUST always exist. It has a variable number of columns.
...
do
		...
		set +x; echo -e "\n\n fastqcflag=$fastqcflag. need to prepare the output folder for these results" >&2; set -x;
            ...
            failedlog=$AlignOutputDir/logs/FAILED_align.${sample} 
		...
		then                
                ...
                echo "$scriptdir/fastqc.sh $runfile $fastqcdir $FastqcOutputDir $fastqcparms $fastqc_input $FastqcOutputDir/logs/log.Fastqc_${SampleName}.in $FastqcOutputDir/logs/log.Fastqc_${SampleName}.ou $email $FastqcOutputDir/logs/qsub.Fastqc_$SampleName > $FastqcOutputDir/logs/log.Fastqc_${SampleName}.in" > $jobfileFastqc
		...
		then
		    ...
		    jobfileAlign=$AlignOutputDir/logs/bwamem.$SampleName.node$OutputFileSuffix.jobfile
		    ...
		    then
			  ...
			  echo "nohup $scriptdir/bwamem_and_markduplicates.sh $AlignOutputDir $fqfiles $alignedbam $dedupbam $RGparms  $runfile $AlignOutputDir/logs/log.bwamem.$SampleName.node$OutputFileSuffix.in $AlignOutputDir/logs/log.bwamem.$SampleName.node$OutputFileSuffix.ou $email $jobfileAlign $failedlog > $AlignOutputDir/logs/log.bwamem.$SampleName.node$OutputFileSuffix.in" > $jobfileAlign
			  ...
			  echo "nohup $scriptdir/bwamem_only.sh $AlignOutputDir $fqfiles $alignedbam $dedupbam $RGparms  $runfile $AlignOutputDir/logs/log.bwamem.$SampleName.node$OutputFileSuffix.in $AlignOutputDir/logs/log.bwamem.$SampleName.node$OutputFileSuffix.ou $email $jobfileAlign $failedlog > $AlignOutputDir/logs/log.bwamem.$SampleName.node$OutputFileSuffix.in" > $jobfileAlign
			  jobfilename=$( basename $jobfileAlign )
			  echo "$AlignOutputDir/logs $jobfilename" >> $AlignOutputLogs/AlignAnisimov.joblist
			  echo "nohup $scriptdir/markduplicates.sh $AlignOutputDir $fqfiles $alignedbam $dedupbam $RGparms  $runfile $AlignOutputDir/logs/log.Markdup.$SampleName.node$OutputFileSuffix.in $AlignOutputDir/logs/log.Markdup.$SampleName.node$OutputFileSuffix.ou $email $jobfileMarkdup $failedlog > $AlignOutputDir/logs/log.Markdup.$SampleName.node$OutputFileSuffix.in" > $jobfileMarkdup
...
then
           ...
           "LAUNCHER")
              ...
              then
                  ...
                  echo "$run_cmd $numalignnodes -N 1 -d $thr ~anisimov/scheduler/scheduler.x $FastqcOutputLogs/FastqcAnisimov.joblist /bin/bash > $FastqcOutputLogs/FastqcAnisimov.joblist.log" >> $qsubFastqcLauncher
             ...
             echo "$run_cmd $numalignnodes -N 1 -d $thr ~anisimov/scheduler/scheduler.x $AlignOutputLogs/AlignAnisimov.joblist /bin/bash > $AlignOutputLogs/AlignAnisimov.joblist.log" >> $qsubAlignLauncher
              ...
              then
                 ...
                 echo "$run_cmd $numalignnodes -N 1 -d $thr ~anisimov/scheduler/scheduler.x $AlignOutputLogs/MarkdupsAnisimov.joblist /bin/bash > $AlignOutputLogs/MarkdupsAnisimov.joblist.log" >> $qsubMarkdupLauncher
